...
HTML 视觉格式、DOM 结构、CSS 类名与卡片结构详见：

- `references/html-template-v5.5.md`

执行本 Skill 时必须读取并遵守该模板文件。

核心原则：该模板只规定视觉格式和 DOM 结构，不规定具体分析结论。赛事选择、方向判断、比分筛选、金额分配和选择理由必须由当前 AI 基于官方 API 数据自主决策。
